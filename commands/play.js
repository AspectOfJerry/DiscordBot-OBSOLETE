...
module.exports = {
    ...
    async execute(message, args, cmd, client, Discord) {
        // const commandOnHold = new Discord.MessageEmbed()
        //     .setColor('#ff0000')
        //     .setThumbnail(`${message.author.displayAvatarURL({dynamic: true, size: 32})}`)
        //     .setTitle('Command disabled')
        //     .setDescription('The play command was put on hold since it is not used. The code is still here can the command can be re-enabled at any time.')
        //     .setFooter('The command is disabled')

        // message.channel.send(commandOnHold)
...
const video_player = async (guild, song, Discord, message, client) => {
...
const skip_song = (message, server_queue, Discord, client) => {
...
const stop_song = (message, server_queue, voice_channel, Discord, client) => {
    const requireUseBeInVC = new Discord.MessageEmbed()
        .setColor('ff0000')
        .setTitle('Error')
        .setDescription('You need to be in a voice channel to execute this command!')
...
const join = (message, voice_channel, Discord, client) => {
    try {
        voice_channel.join()
