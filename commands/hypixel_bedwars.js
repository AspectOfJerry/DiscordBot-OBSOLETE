...
module.exports = {
    name: 'hypixel_bedwars',
    description: 'Usage: "%hypixel_bedwars <userName>"',
    async execute(message, args, cmd, client, Discord) {
        ...
        let player_bedwars_
        let player_bedwars_four_three_final_deaths
            ...
            .then(data => {
                if(data.playe) {
                    ...
                    function playerRank() {
                        ...
                        if(data.player.rank) {
                            if(data.player.id == '516398d00cf273d9c97152c3' || data.player.id == '516398d30cf273d9c97152c4') {
                                player_rank = "[OWNER] "
        ...
        setTimeout(() => {
            ...
            if(!args[0]) {
                const errorRequireUserName = new Discord.MessageEmbed()
                    .setColor('#ff0000')
                    .setThumbnail(`${message.author.displayAvatarURL({dynamic: true, size: 16})}`)
                    .setDescription('error: You must provide a player name.')
                
                message.channel.send(errorRequireUserName)
                return;
            }
            // if(!args[2]) {
            //     if(!args[1]) {
